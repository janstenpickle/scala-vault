...
case class Secrets(config: VaultConfig, backend: String) {
  ...
  def set(key: String, value: String)(implicit ec: ExecutionContext): Task[WSResponse] =
    set(key, "value", value)

  def set(key: String, subKey: String, value: String)(implicit ec: ExecutionContext): Task[WSResponse] =
    set(key, Map(subKey -> value))

  def set(key: String, values: Map[String, String])(implicit ec: ExecutionContext): Task[WSResponse] =
  ...
  def list(implicit ec: ExecutionContext): Task[List[String]] =
    config.authenticatedRequest(backend)(_.withQueryString("list" -> true.toString).get()).
      acceptStatusCodes(200).
